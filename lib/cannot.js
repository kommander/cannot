 ...
 * @api public
 */
function Cannot(action, subject, reason) {
  if (!(this instanceof Cannot)) {
    return new Cannot(action, subject, reason);
  }
  ...
  if (!this._action || !this._subject) {
    throw this.constructor('create', 'exception', 'action or subject is missing');
  }
...
Cannot.prototype._generateMessage = function _generateMessage() {
  if (!this._message) {
    this._message = [this.object, ' could not ', this._action, ' ', this._subject].join('');
 ...
 * @api public
 */
Cannot.prototype.__defineGetter__('action', function action() {
 ...
 * @api public
 */
Cannot.prototype.__defineGetter__('subject', function subject() {
 ...
 * @api public
 */
Cannot.prototype.__defineSetter__('object', function objectSetter(object) {
  this._object = object;
});
