...
function Cannot(verb, object, reason) {
  ...
  this.is.cannot = this.assert.cannot.bind(this);
}
...
Cannot.prototype.__defineGetter__('reason', function reasonGetter() {
  ...
  return this._reason && this._reason.code ? this._reason.code : this._reason;
});
...
Cannot.prototype.is = function is(verb, object) {
  return assertIs(this, verb, object);
};
...
Cannot.prototype.__defineGetter__('assert', function assert() {
  ...
  return {
    cannot: (verb, object) => {
      const isCannot = assertIs(self, verb, object);
      const becauseInterface = {
        because: function assertBecause(reason) {
          return isCannot ? self.reason === codify(reason) : false;
        },
      };

      becauseInterface.because.cannot = (subVerb, subObject) =>
        this && this.is(subVerb, subObject);

      return becauseInterface;
    },
  };
});
